<?php
	...
	$insertar3="SELECT `Precio_tramite` FROM `servicios` WHERE id_servicio=5";
	...
	$_SESSION['datos'] = 2;
	$_SESSION['trami'] = "tramitesReemplacamiento";
				...
				<form  id="formuTramites">
					<div id="columnas">
						<div class="columnsP">
							...
							<div id="field placa" class="field input-field placa">
								<label for="formFileSm" class="form-label">Número de Placa:</label>
								<input type="text" placeholder="" class="caja" name="numPlaca" id="numPlaca">
							</div>
						</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$('form').submit(function(e){
			...
			if (placa=="") {
				document.getElementById("field placa").classList.add("error");
			}else{
				document.getElementById("field placa").classList.remove("error");
			}
			if ((idOficial.files.length == 0) || (FacVehi.files.length == 0) || (comprDomi.files.length == 0)) {
				swal('¡Atención!', 'Los archivos no han sido seleccionado.', 'error');
			}
			if((nombreC != "") && (telefono != "") && (placa != "") && (idOficial.files.length > 0) && (FacVehi.files.length > 0) && (comprDomi.files.length > 0)){
				var valores = new FormData(formulario);
				valores.append('tramite', 'Reemplacamiento');
				$.ajax({								
									...
									$.ajax({
										...
										function(datos){
											...
											btnPDF.onclick = function(){
												window.open("../tickets.php?user="+nombreC+"&tram=Reemplacamiento.pdf");
											}
									        ...
									        btnsubmit.onclick = function(){
									            ...
									            var qrcode = new QRCode(qrfinal, {
									                text: "http://gestoriae.uptex-vsc2.com/tickets.php?user="+nombreC+"&tram=Reemplacamiento.pdf",
									                width: 200,
									                height: 200,
									                colorDark: "#000000",
									                colorLight: "#ffffff",
