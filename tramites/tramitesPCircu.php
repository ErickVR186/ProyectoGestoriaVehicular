<?php
	...
	$insertar3="SELECT `Precio_tramite` FROM `servicios` WHERE id_servicio=2";
	...
	<div id="main">
		...
		<div class="form login">
			<div class="form-content">
				<header>Permiso de Circulación</header>
				<form id="formuTramites">
					<div id="columnas">
						...
						<div id="columna2" class="columnsP">
							...
							<div class="file input-file">
								<label for="formFileSm" class="form-label">Acreditación Juridica:</label>
								<input type="file" class="form-control form-control-sm caja" name="AcredJuri" id="AcredJuri">
								<b id="tooltip1" class="tooltip1">
									<i class='bx bxs-help-circle'></i>
									<span class="tooltip1-box">Subir documentación en formato pdf</span>
								</b>
							...
							<div class="file input-file">
								...
								<input type="file" class="form-control form-control-sm caja" name="comprDomi" id="comprDomi">
								<b id="tooltip1" class="tooltip1">
									<i class='bx bxs-help-circle'></i>
									<span class="tooltip1-box">Subir documentación en formato pdf</span>
								</b>
							...
							<div class="file input-file">
								<label for="formFileSm" class="form-label">Comprobante de Propiedad:</label>
								<input type="file" class="form-control form-control-sm caja" name="comprProp" id="comprProp">
								<b id="tooltip1" class="tooltip1">
									<i class='bx bxs-help-circle'></i>
									<span class="tooltip1-box">Subir documentación en formato pdf</span>
								</b>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$('form').submit(function(e){
			...
			var AcredJuri = document.getElementById('AcredJuri');
			var comprDomi = document.getElementById('comprDomi');
			var comprProp = document.getElementById('comprProp');
			...
			if ((idOficial.files.length == 0) || (AcredJuri.files.length == 0) || (comprDomi.files.length == 0) || (comprProp.files.length == 0) || (CompPagoT.files.length == 0)) {
				swal('¡Atención!', 'Los archivos no han sido seleccionado.', 'error');
			}
			if((nombreC != "") && (telefono != "") && (idOficial.files.length > 0) && (comprDomi.files.length > 0) && (comprProp.files.length > 0) && (CompPagoT.files.length > 0)){
				var valores = new FormData(formulario);
				valores.append('tramite', 'Permiso de Circulacion');
				$.ajax({								
									...
									$.ajax({
										...
										function(datos){
											...
											btnPDF.onclick = function(){
												window.open("../tickets.php?user="+nombreC+"&tram=Permiso de Circulacion.pdf");
											}
									        ...
									        btnsubmit.onclick = function(){
									            ...
									            var qrcode = new QRCode(qrfinal, {
									                text: "http://gestoriae.uptex-vsc2.com/tickets.php?user="+nombreC+"&tram=Permiso de Circulacion.pdf",
									                width: 200,
									                height: 200,
									                colorDark: "#000000",
									                colorLight: "#ffffff",
