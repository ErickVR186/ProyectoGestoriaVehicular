						...
						<div id="columnas">
							<div class="columnsP">
								<div id="field nombres" class="field input-field nombres">
									...
									<input type="text" placeholder="" class="caja" name="nombre" id="nombre" value="<?php echo $nombreC ?>" readonly="readonly">
								</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$('form').submit(function(e){
			...
			if ((idOficial.files.length == 0) || (tarjCircu.files.length == 0)) {
				swal('¡Atención!', 'Los archivos no han sido seleccionado.', 'error');
			}
			if((nombreC != "") && (telefono != "") && (idOficial.files.length > 0) && (tarjCircu.files.length > 0)){
				var valores = new FormData(formulario);
				valores.append('tramite', 'Reposicion de Tarjeta de Circulacion');
				$.ajax({								
									...
									$.ajax({
										...
										function(datos){
											...
											btnPDF.onclick = function(){
												window.open("../tickets.php?user="+nombreC+"&tram=Reposicion de Tarjeta de Circulacion.pdf");
											}
									        ...
									        btnsubmit.onclick = function(){
									            ...
									            var qrcode = new QRCode(qrfinal, {
									                text: "http://gestoriae.uptex-vsc2.com/tickets.php?user="+nombreC+"&tram=Reposicion de Tarjeta de Circulacion.pdf",
									                width: 200,
									                height: 200,
									                colorDark: "#000000",
									                colorLight: "#ffffff",
