<?php
	...
	$insertar3="SELECT `Precio_tramite` FROM `servicios` WHERE id_servicio=8";
	...
	$_SESSION['datos'] = 2;
	$_SESSION['trami'] = "tramitesLicencia";
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$('form').submit(function(e){
			...
			if ((idOficial.files.length == 0) || (comprDomi.files.length == 0)) {
				swal('¡Atención!', 'Los archivos no han sido seleccionado.', 'error');
			}
			if((nombreC != "") && (telefono != "") && (idOficial.files.length > 0) && (comprDomi.files.length > 0)){
				var valores = new FormData(formulario);
				valores.append('tramite', 'Licencia');
				$.ajax({								
									...
									$.ajax({
										...
										function(datos){
											...
											btnPDF.onclick = function(){
												window.open("../tickets.php?user="+nombreC+"&tram=Licencia.pdf");
											}
									        ...
									        btnsubmit.onclick = function(){
									            ...
									            var qrcode = new QRCode(qrfinal, {
									                text: "http://gestoriae.uptex-vsc2.com/tickets.php?user="+nombreC+"&tram=Licencia.pdf",
									                width: 200,
									                height: 200,
									                colorDark: "#000000",
									                colorLight: "#ffffff",
